...
export default async function SolicitacoesPage() {
  ...
  if (isAdmin) {
    ...
    const [{ data: todasSolicitacoes }, { data: lideresData }, { data: pedidosData }] = await Promise.all([
      (adminClient as any)
        ...
        .select('id, nome, telefone, email, idade, estado_civil, tem_filhos, filhos_detalhes, conjuge_nome, conjuge_telefone, conjuge_idade, bairro, tipo_membro, melhor_dia, status, criado_em, lider_encaminhado_id, celula_id, user_id')
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto pb-8">
        ...
        <section>
          ...
          <SolicitacoesPanel solicitacoes={solicitacoes} lideres={lideres} podeConfirmar />
        </section>
  ...
  const { data: todasSolicitacoes } = await (supabase as any)
    .from('solicitacoes_celula')
    .select('id, user_id, celula_id, nome, telefone, email, idade, bairro, melhor_dia, estado_civil, tem_filhos, filhos_detalhes, conjuge_nome, conjuge_telefone, conjuge_idade, tipo_membro, criado_em, status')
