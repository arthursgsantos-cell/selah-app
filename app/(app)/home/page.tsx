...
import { CalendarDays, MapPin, ChevronRight, Users, Sparkles, Cake, History, Bell, GraduationCap, HeartHandshake } from 'lucide-react'
...
const funcaoLabels: Record<string, string> = Object.fromEntries(
  FUNCOES_ESCALA.map((f) => [f, funcaoComEmoji(f)])
)
...
export default async function HomePage() {
  ...
  const [{ data: eventos }, { data: profilesIgreja }, { data: ultimosEncontrosCelula }, { data: ultimosEventosPast }, { data: pastorProfiles }, { data: fotosComunidade }, { data: encontroFotos }, { data: destaquesData }] = await Promise.all([
    ...
    igrejaId
      ? admin
          .from('eventos')
          .select('id, slug, titulo, data_hora, local, imagem_url, capa_pagina_url')
          .eq('igreja_id', igrejaId)
          .eq('destaque', true)
          .gte('data_hora', new Date().toISOString())
          .order('data_hora', { ascending: true })
  ...
  // A capa da página tem prioridade sobre o card do evento: é a arte larga,
  // sem texto, feita justamente para ocupar um banner.
  const destaques: EventoDestaque[] = (
    (destaquesData ?? []) as unknown as {
  ...
  // O visualizador recebe a assinatura junto: célula, rede e data viram a
  // marca d'água sobre a foto.
  const galleryPhotos = galleryItems.length > 0
    ? galleryItems.map((g) => ({ url: g.src, celula: g.celula, rede: g.rede, data: g.date }))
    : staticPhotos.map((url) => ({ url }))
  ...
  if (!profile) {
    ...
    return (
      <div className="space-y-4 max-w-2xl mx-auto pb-8">
        ...
        {/* Culto ao vivo — logo abaixo do herói, que é onde a pessoa procura
