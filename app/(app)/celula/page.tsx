...
export default async function MinhasCelulaPage() {
  ...
  const [
    ...
    admin
      .from('celulas')
      .select('id, nome, descricao, logo_url, frequencia, local_padrao')
      .eq('id', celulaId)
      .single(),
    admin
      .from('celula_membros')
  ...
  const [{ data: dependentesData }, { data: presencasMembros }] = await Promise.all([
    membroIds.length
      ? admin.from('dependentes').select('profile_id, nome, data_nascimento, tipo').in('profile_id', membroIds)
    ...
    if (dep.data_nascimento) {
      ...
      if (d <= 6) {
        ...
        aniversariantesSemana.push({
          nome: dep.nome,
          subtitulo: `${dep.tipo === 'cônjuge' ? 'Cônjuge' : 'Filho(a)'}${membroNome ? ` de ${membroNome}` : ''}`,
          avatar_url: null,
          daysUntil: d,
        })
      }
              ...
              return (
                <div key={i} className="flex items-center gap-3 bg-white/20 rounded-xl p-2.5">
                  <div className="h-9 w-9 rounded-full bg-white/30 flex items-center justify-center text-sm font-bold shrink-0 overflow-hidden">
                    {a.avatar_url
                      ? <img src={a.avatar_url} alt={a.nome} className="h-full w-full object-cover rounded-full" />
                      : ini}
                  </div>
                  <div className="flex-1 min-w-0">
