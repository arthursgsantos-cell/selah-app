...
interface Props {
  ...
  /** Dia/horário padrão da célula, usados para sugerir a data do próximo encontro. */
  diaSemana?: number | null
  horarioPadrao?: string | null
  dependentes?: Dependente[]
  celulaColor?: string | null
  fotosInit?: { id: string; url: string; criado_em?: string | null }[]
  /** Nome da rede, usado na marca d'água do visualizador. */
  redeNome?: string | null
  canUpload?: boolean
  /** Datas futuras projetadas + escalas já definidas, para a aba Calendário. */
  datasCalendario?: DataCalendario[]
  escalasCalendario?: EscalaCalendario[]
  canEditEscala?: boolean
  /** Pessoas já atribuídas à célula que ainda não criaram conta. */
...
function buildFamilias(membros: Membro[], dependentes: Dependente[]): FamiliaGroup[] {
  ...
  for (const m of membros) {
    ...
    // Sem subtítulo "Cônjuge": quando os dois lados têm conta própria (e-mail
    // vinculado), não há como decidir de forma não-arbitrária quem é "o
    // titular" e quem é "o cônjuge" — o agrupamento "Nome A & Nome B" já
    // deixa a relação clara sem precisar da tag em cada linha.
    ...
    // Quando é casal, o cabeçalho "Nome A & Nome B" já identifica os pais —
    // repetir "de Fulano" citaria só um dos dois e ficaria incompleto/errado.
    const ehCasal = !!(conjugeMembro || conjugeDep)

    for (const f of filhos) {
      if (!f.data_nascimento) continue
      const parentPrimeiro = membros.find((x) => x.user_id === f.profile_id)?.nome?.split(' ')[0]
      const rotulo = f.sexo === 'M' ? 'Filho' : f.sexo === 'F' ? 'Filha' : 'Filho(a)'
      rows.push(makeRow('aniversario', f.nome, f.data_nascimento, {
        subtitulo: ehCasal || !parentPrimeiro ? rotulo : `${rotulo} de ${parentPrimeiro}`,
      }))
    }
    ...
    <div className={`flex items-center gap-3 px-3 py-2.5 ${isThisMonth ? 'bg-amber-50/50' : 'bg-background'}`}>
      <div className="h-8 w-8 rounded-full shrink-0 overflow-hidden flex items-center justify-center">
        {row.tipo === 'casamento' ? (
          ...
          <img referrerPolicy="no-referrer" src={row.avatar_url} alt={row.nome} className="h-full w-full object-cover" />
        ) : (
...
export function CelulaTabs({
  ...
  return (
    <div className="rounded-2xl bg-card/80 backdrop-blur-sm border border-border/60 shadow-sm p-4">
      ...
      <div className="border-b border-border overflow-x-auto no-scrollbar">
        <div className="flex items-end justify-between min-w-max pr-0.5">
          <TabsList variant="line" className="h-auto pb-0 gap-0 border-b-0 flex-shrink-0">
            ...
            <TabsTrigger value="calendario" className={triggerCls}>Calendário</TabsTrigger>
            <TabsTrigger value="membros" className={triggerCls}>
          ...
          {/* Flutuante: sai da linha das abas e passa a acompanhar a rolagem,
              devolvendo espaço para os nomes das abas. */}
          <CriarEncontroDialog
            celulaId={celulaId}
      ...
      <TabsContent value="calendario" className="mt-4">
        <CalendarioCelulaTab
          ...
          escalas={escalasCalendario}
          membros={membros.map((m) => ({ user_id: m.user_id, nome: m.nome }))}
          canEdit={canEditEscala}
        />
      </TabsContent>

      {/* ── Membros ── */}
      <TabsContent value="membros" className="mt-4 space-y-4">
        <PreCadastradosCelula
          preCadastrados={preCadastrados}
          celulas={celulasDisponiveis}
          canEdit={canEditEscala}
        />

        {membros.length === 0 && preCadastrados.length > 0 ? null : membros.length === 0 ? (
          <div className="py-10 text-center">
