...
import {
  atualizarAparenciaCelulaAction,
  uploadFundoCelulaAction,
  atualizarFundoGaleriaCelulaAction,
  alternarAutoCorCelulaAction,
  salvarAutoCorCelulaAction,
} from '@/app/actions/celula'
...
export function CelulaFundoPagina({
  celulaId,
  cor: corInicial,
  ...
  function salvar(novoFundo: FundoTipo, novaCor: string, novaCorSec: string, novaOpacidade = opacidade) {
    ...
    startTransition(async () => {
      try {
        await atualizarAparenciaCelulaAction(celulaId, {
          cor: novaCor,
  ...
  function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      try {
        const url = await uploadFundoCelulaAction(celulaId, fd)
        setImagem(url)
        await atualizarAparenciaCelulaAction(celulaId, {
          cor,
          cor_secundaria: corSec,
      ...
      {canEdit && (
        <AutoCorCapa
          ...
          salvar={async (c, cs, origem) => {
            await salvarAutoCorCelulaAction(celulaId, { cor: c, corSecundaria: cs, origem })
          }}
        />
      )}
          ...
          {aberto && (
            <div className="absolute top-full right-0 mt-2 z-30 w-72 rounded-2xl border border-border bg-card p-4 space-y-4 shadow-xl">
              <AutoCorControle
                ...
                alternar={async (a) => {
                  setAutoCor(a)
                  await alternarAutoCorCelulaAction(celulaId, a)
                }}
              />
              ...
              <FundoGaleriaControle
                ...
                salvar={(ativo, opacidade) =>
                  atualizarFundoGaleriaCelulaAction(celulaId, { ativo, opacidade })
                }
              />
