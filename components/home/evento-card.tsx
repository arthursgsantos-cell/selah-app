...
import { CalendarDays, MapPin, X, Check, Users, Heart, ClipboardList, ExternalLink } from 'lucide-react'
...
interface Evento {
  id: string
  /** URL legível da página do evento. Ausente cai no id. */
  slug?: string | null
  titulo: string
...
export function EventoCard({ evento, minhaResposta: minhaRespostaInit, totalVou: totalVouInit, redeNome, totalLikes: totalLikesInit = 0, euCurtei: euCurteiInit = false, minhaInscricao, editButton }: Props) {
  ...
  return (
    <>
      {/* Precisa ser <div>, não <button>: o card contém outros controles
          clicáveis (curtir, editar) e HTML não permite botão dentro de botão
          — isso causava erro de hidratação. */}
      <div
        role="button"
        tabIndex={0}
        onClick={() => setAberto(true)}
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setAberto(true) } }}
        className="w-full text-left flex gap-3 p-4 rounded-2xl border border-border bg-card hover:bg-accent/30 transition-colors cursor-pointer"
      >
        {evento.imagem_url ? (
      ...
      <Dialog open={aberto} onOpenChange={setAberto}>
        <DialogContent showCloseButton={false} className="p-0 overflow-hidden max-w-sm w-full gap-0">
          ...
          <div className="p-4 space-y-3">
            ...
            {/* Página completa do evento (fotos do local, vídeo, detalhes) */}
            <Link
              href={`/evento/${evento.slug ?? evento.id}`}
              className="flex items-center justify-center gap-1.5 text-sm font-medium h-9 rounded-xl border border-border hover:bg-accent transition-colors"
            >
            ...
            <div className="border-t border-border/60 pt-3 space-y-2">
              ...
              ) : evento.tipo_inscricao === 'link' && evento.link_inscricao_url ? (
                <a
                  href={evento.link_inscricao_url}
                  target="_blank"
                  rel="noopener noreferrer"
