...
/**
 * Fundo da página do evento — mesma mecânica de redes e células.
 *
 * Diferença importante: `fundoTipo` nulo significa "sem personalização", e a
 * página mantém o fundo normal do app. Só quem escolhe um estilo passa a ter
 * a camada de fundo. É o que evita mudar a aparência dos eventos antigos.
 */
export function EventoFundo({
  eventoId,
  cor: corInicial,
  ...
  function salvar(
    ...
    startTransition(async () => {
      try {
        await atualizarFundoEventoAction(eventoId, {
          cor: novaCor,
  ...
  function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        setImagem(await uploadFundoEventoAction(eventoId, fd))
      } catch (err) {
        setImagem(anterior)
    ...
    <>
      ...
      {canEdit && (
        <AutoCorCapa
          ...
          salvar={async (cor, corSecundaria, origem) => {
            await salvarAutoCorEventoAction(eventoId, { cor, corSecundaria, origem })
          }}
        />
      )}
          ...
          {aberto && (
            <div className="absolute top-full right-0 mt-2 z-30 w-72 rounded-2xl border border-border bg-card p-4 space-y-4 shadow-xl">
              <AutoCorControle
                ...
                alternar={async (a) => {
                  setAutoCor(a)
                  await alternarAutoCorEventoAction(eventoId, a)
                }}
              />
              ...
              <FundoGaleriaControle
                ...
                salvar={(ativo, opacidade) =>
                  atualizarFundoGaleriaEventoAction(eventoId, { ativo, opacidade })
                }
              />
